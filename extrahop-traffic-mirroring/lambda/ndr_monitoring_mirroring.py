...
# Global constant for resource tagging
YOR_TRACE = "B7DFB137-E7F6-448C-9160-132AE9FAC7FB"
...
def create_traffic_mirror_session(ec2_client, traffic_mirror_target_id, traffic_mirror_filter_id, item, session_number=1):
    ...
    try:
        ...
        if "NetworkInterfaceNotSupported" in error_msg:
            print(f"⚠️  Instance type not supported for traffic mirroring")
        elif "already exists" in error_msg.lower():
            print(f"ℹ️  Traffic mirror session already exists")
...
def handler(event, context):
    ...
    for instance in describe_instances_in_vpc(
        ...
        if not instance_type_supported(instance):
            print(f"⚠️  Instance {instance['InstanceId']} type {instance['InstanceType']} not supported")
            continue
        ...
        for tag in instance_tags:
            if not tag['Key'].startswith('aws:') and tag['Key'] not in ['yor_trace', 'instanceId']:
                session_tags.append(tag)
        
        # Add required tags
        session_tags.append({"Key": "instanceId", "Value": instance["InstanceId"]})
        session_tags.append({"Key": "yor_trace", "Value": YOR_TRACE})
